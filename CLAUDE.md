...
- **Lanzamiento: sábado 26 de septiembre de 2026, 16.30 hs, Plaza de El Gigante.** Fecha límite dura. (Se movió del 21/09 —Día de la Primavera, día de semana— al sábado 26 siguiente para tener a los vecinos disponibles en fin de semana; sigue cayendo en la semana de la primavera y el arranque de la temporada de riego. Es el evento de lanzamiento / "bautismo de la plaza".) **Estado real (actualizado 17-jul-2026): el MVP ya está terminado y en producción** — se desarrolló en ~12 días (4 al 16 de julio). El encuadre de "apretados de tiempo" del arranque quedó viejo: ya NO hay urgencia de calendario. El foco ahora es pulir y validar con las comisiones; no cortar features por tiempo. (El dominio ya se decidió: queda el subdominio de Vercel — ver decisión 23.) (Nota histórica: al iniciar, el 4-jul, se estimaron ~2,5 meses y eso llevó a cortar un MVP chico; la estimación resultó holgada.)
...
13. **Stack elegido: Supabase (plan gratis) + PWA en JS + Web Push.** Se evaluó hacerlo en PHP (lo que Pedro ya sabe) sobre su propio servidor, pero se descartó: alojar PHP gratis y confiable no existe, y el servidor casero no sirve para una app pública (IP dinámica, CGNAT, tiempo activo — un QR muerto en la plaza mata el proyecto). el plan gratis de Supabase cubre de sobra la escala (una plaza). Definiciones que acompañan:
    - **Frontend:** PWA en **SvelteKit** (decidido jul 2026; se evaluó Vite+React pero Svelte gana por bundles chicos para mala conectividad, legibilidad viniendo de PHP y convenciones de fábrica). Inevitable: service worker, sin conexión y notificaciones viven en el navegador sea cual sea el backend.
    - **Arquitectura del código: ver [docs/arquitectura.md](docs/arquitectura.md)** (organización por features, reglas del juego como funciones puras en `domain/`, integridad/anti-trampa en Postgres vía RPC + RLS, migraciones versionadas, dominio en español). Cambios a ese doc se conversan antes.
...
18. **Los pines físicos se entregan validando un QR de canje.** Cuando un vecino gana una insignia que es pin real, la app le muestra un QR de canje único. El que entrega el pin (comisión) lo escanea con una herramienta validadora nuestra (página admin): si es válido, entrega el pin y el sistema lo marca como canjeado — imposible cobrarlo dos veces, sin listas en papel. El esquema de base lo contempla desde el día 1 (insignia ganada → canje pendiente/entregado). **Decidido (jul 2026): la herramienta validadora ENTRA AL MVP** — en el evento del 26/09 se entregan pines en vivo y el canje tiene que ser a prueba de dobles desde el primer día.
19. **El riego tiene una duración mínima ("Regando…" con barra de progreso) que además educa.** Al confirmar, corre unos segundos mostrando un tip sobre el árbol / cómo regar bien, y recién al terminar registra. Doble función: refuerza el anti-trampa (no se puede escanear-y-listo masivo, cada riego lleva su tiempo real, más allá del tiempo de espera) y aprovecha el momento para enseñar. (En el MVP definir la duración real.)
...
21. **El estilo visual final es el de la demo** (decidido jul 2026): pixel art oscuro, paleta violeta, tipografías Press Start 2P (títulos) y VT323 (lectura), y el **árbol voxel isométrico** que ya vive en `tools/demo.html` (función `isoTree`). Es lo que aprobó la comisión de cultura, así que no se rediseña: se porta. Clave: el árbol **no necesita assets** — se genera por código a partir de la salud (0..1), interpolando el color de las hojas y sumando bloques de follaje y flores violetas a medida que mejora. **Estado (actualizado ago 2026): la deuda visual YA está saldada y en producción.** El estilo de la demo se portó entero — pixel art, paleta violeta, las dos tipografías, y el árbol voxel generado por código (`src/lib/ui/ArbolVoxel.svelte` + `src/lib/domain/arbol-voxel.ts`), que en la app va incluso más lejos que la demo (reacciona al balance hídrico real). Los tickets BT-29 (árbol) y BT-30 (sistema visual) están hechos. Lo de "placeholder funcional" quedó viejo: la app ES el estilo aprobado.

22. **Identidad física del árbol: QR primario; marca tallada evaluada y descartada.** El QR único en chapita sigue siendo la identidad y el escaneo del circuito (está hecho, es lo más confiable del stack). Se evaluó reemplazarlo/respaldarlo con un código tallado en el tutor (sistema "ESPIGA") leído por la cámara de la app: **descartado para el MVP y el circuito** — mete un lector de visión propio, poco confiable a la intemperie (peor bajo la sombra moteada de los jacarandás), en el camino crítico del riego, y resuelve un problema (robo/desgaste de chapitas) todavía no probado a escala de una plaza. Además, sus dos razones de ser —tallable a mano y legible sin app— acá no aplican: tenemos con qué fabricar chapitas y controlamos la app. **El respaldo ante una chapita perdida es la geoposición del árbol, que ya tenemos del relevamiento** ([docs/relevamiento.md](docs/relevamiento.md)): la app usa el GPS del teléfono para ofrecer los árboles de alrededor y el vecino/admin confirma cuál — sin trabajo de campo nuevo. (Los jacarandás están a ~7 m entre sí y el GPS no distingue a esa distancia — por eso la proximidad del anti-trampa es de ~50 m —, así que el respaldo es "elegí de esta lista corta de acá cerca", no automático; alcanza para un caso ocasional.) ESPIGA escaneable = idea de v2/experimento, solo si se escala a muchas plazas o el robo de chapitas se vuelve un problema medido. No reabrir sin datos nuevos.

23. **Dominio: se queda en `arboles-gigantes.vercel.app`, sin dominio propio** (decidido 21-jul-2026). Se evaluó comprar dominio (.com.ar $8.500/año era el candidato) y se descartó por presupuesto: la infra lleva $0.00 gastados y se mantiene el invicto. Análisis de riesgo de Pedro: es más probable no poder pagar la renovación del dominio en 3 años (dominio impago = QRs muertos) que que Vercel elimine los subdominios gratis en ese plazo. Mitigación de fondo: si algún día hay que migrar, reemplazar QRs = reimprimir chapitas (barato, ya contemplado en qr/README.md). Consecuencias operativas: (a) **NO renombrar el proyecto en Vercel** — el nombre del proyecto ES el dominio ahora; renombrarlo mata todos los QRs impresos; (b) la cuenta de Vercel de Pedro pasa a ser infraestructura crítica (2FA obligatorio); (c) los QRs definitivos se generan desde el admin contra `https://arboles-gigantes.vercel.app` (los de `qr/jacarandas/` apuntan a la demo vieja de github.io — NO imprimirlos). No reabrir salvo que cambie el presupuesto o Vercel cambie las condiciones del plan Hobby.
...
Criterio de corte: **¿qué tiene que poder hacer la app el 26/09 en la plaza?** Los vecinos hacen su primer riego en vivo; la app tiene que escanear, registrar, dar algo que se sienta, y que un par se lleven un pin. Todo lo que no sirva a eso, afuera.
...
- **Canje de pines con QR:** el ganador muestra su QR de canje, el validador (página admin) lo escanea y marca entregado. Decidido jul 2026: entra al MVP — es la mecánica de entrega del evento.
...
1. **MVP (26/09, lanzamiento):** circuito + identidad + pantalla inicial + puntos + 2–3 pines + admin.
